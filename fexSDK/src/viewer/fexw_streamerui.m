...
%    timestamp for the frame currently displayed. The play/pause button can
%    be trigger using the combination 'Command+P' or 'Ctrl+P'.
%
...
%    video, or to zoom in (5 min,1 min, or 30 sec). You can switch between
%    full x-axis mode to 30 seconds display using the combination
%    "Command+B" (or "Ctrl + B" on non-OSX system).
...
%
%
% ACCELLERATORS & SHORTCUTS:
%
%
% - 'ESC': Closes the FEXW_STREAMERUI;
% - 'COMMAND+P' and 'CTRL+P': Pauses and Plays the video;
% - 'COMMAND+B': changes the span of the x-axis on the plots [this is
%       CTRL+N on non-OSX systems];
% - 'COMMAND+N': Starts a new note [this is CTRL+N on non-OSX systems].
%
%
...
%
% Add accellerators that apply to the entire UI
% Escape routine
...
if strcmp(eventdata.Key,'escape')
    ...
    uiresume(hObject);
elseif ismember(eventdata.Modifier{1},{'command','control'}) && strcmpi(eventdata.Key,'p')
    if strcmp(get(handles.PlayButton,'String'),'Pause')
...
% Update image when UI is paused
if strcmpi(get(handles.PlayButton,'String'),'play')
   img = FormatFrame(handles);
   imshow(img,'parent',handles.FrameAxis);
end
...
% 
% This can also be called using the Command+B combination. When full axis
% was already selected, this command will prompt 30 seconds axis extent

hand_sel = get(handles.MT_xaxisextent,'Children');
% Case in which it was already selected this deselects it (unless the video
% is shorter than 30 seconds).
if strcmp(get(handles.MT_XaxisLength,'Checked'),'on') && strcmp(get(handles.MTX30s,'Enable'),'on')
